...
The W37 edition published on Friday 2026-09-11 at 20:52 MX (patched 20:58) was wrong and was re-issued on
...
are in `9795361` (radar) and `c5394a8` (very-light-cms: verifier + bypass trailer); the Journal re-issue is `e29d163` + commentary `c1d975b` + audit `83bc382`.

## 1. What the pipeline did wrong (not Jarvis's fault)

| # | Defect | Evidence | Fix |
|---|--------|----------|-----|
| P1 | Scan ran on **last week's bars** for 385/387 tickers. Polygon's free tier caps an `sort=asc` response at 104 rows; the 2-year window held 105 weeks and the row dropped was the **newest**. | radar.db after the Friday fetch: `2026-09-04` = 368 rows, `2026-09-11` = 2 rows (PARA, ONC). Live probe archived at `results/probes/2026-09-12-polygon-sort-cap.md`: `sort=asc` → 104 rows, last 2026-08-30-keyed; `sort=desc` → 105 rows incl. 2026-09-06-keyed. | `fetch-polygon.ts` requests `sort=desc` (`buildPolygonAggsUrl`, test pinned). |
| P2 | Nothing checked that fetched bars belonged to the scan week; `fetchAll` "succeeded". | `[1/8] Fetching data...` printed nothing and went straight to `[2/8] Running scan...` — no freshness check in between. | `src/coverage.ts` + `ensureBarCoverage()`: <90% → wait/refetch lagging (cache bypass) ×2 → Telegram + ABORT before scan/push/publish; lagging tickers excluded from the scan. `--refetch` flag. |
| P3 | Scorecard measured each ticker's last two bars, so 21 of the 22 rows reported the **W35→W36** move labelled **W36→W37** (SMMT "+28.2%", MLM "−3.1%"); only PARA had a real W37 bar. | Bars: SMMT 08-28 13.73 → 09-04 17.605; real W37 move −0.3%. | Rows whose last bar ≠ run date, or whose ticker left the universe, print `— \| — \| —` "not measured" and are excluded from the summary and exit analysis. |
| P4 | 20 dead tickers still scored weekly. PARA resolved to **Banzai International** ($1; 220.80 → 1.76 across a 12-month hole in the series, ≈125×) and headlined S1 at p0% for two weeks; K sat in the pre-radar on a June bar. | Polygon reference: PARA = Banzai; K/EA/AVB/EQR/NUVL/IAC/SATS NOT_FOUND; 12 series stopped 2024-10 … 2026-05. | Removed from `universe.ts` (387 → 367), `discarded` in `ticker_registry`, data preserved. |
| P5 | SPY never refreshed after the 07-14 Polygon cutover, so from W30 on the generator could only print `SPY: —` (W32, W34, W36, stale W37). The SPY text in W30 (−0.6%), W31 (+1.2%), W33 (+0.5%) and W35 (prose) was entered by hand. | `MAX(date)` for SPY = 2026-07-17 (W29) in the pre-reissue snapshot; generator emits only `SPY W{p}→W{c}: {pct}` or `SPY: —`. | SPY fetched every week with its own refetch when it lags alone. |
...
| P7 | The commentary verifier flagged acronyms (PD-1 → token `PD`, VEGF, PFS, NSCLC, EU, FDA) as fabricated tickers — the false positives that invited the bypass in J3. | 6 HARD before the fix (recorded in task `bd680654` and in `c5394a8`'s message; not re-runnable, page and CSV since replaced). | very-light-cms `c5394a8`: unknown token is HARD only within ±40 chars of ticker context or written as a symbol; otherwise SOFT. |
...
| J5 | **Invented a roster history**: "recovered FMC and HAL to the S2D roster". HAL has never been S2D; FMC last was in W31 (and W25), not W36 — both were S1 in W35 and filtered by `ranging` in W36. | `grep -H -E '^(FMC\|HAL),[^,]*,[^,]*,S2D' results/radar_2026-W*.csv`. | Check prior CSVs before asserting a ticker's history. |
...
| J9 | **Patched a symptom instead of reporting the bug**: hand-translated the Spanish Exit Analysis in W36 and again in W37 — the fifth manual pass (W25 `c79812a`, W26 `9b5a21d`, W31 `9f552a2`, a bulk pass `6b8c66a`) — without flagging that the generator emits Spanish. | Commits `6fd9fae`, `da55b14`. | A repeat manual fix = a generator bug; report it. |
| J10 | **Re-issued commentary (07:59): three table-derived claims wrong** — "eleven are still holding at structural lows" (11 hold; 5 at lows), "four broke down" (3 breakdowns + 1 false technical tick), pre-radar "weighted toward XLP, XLV, XLY and XLRE" (order is XLP 11, XLY 7, XLC 6, XLV 5, XLRE 5). Sourced figures were all correct; derived counts were not. One derived figure survived the first correction: "two-thirds of your exits are breakdowns" quoted the generator's 4/6 deterioration bucket next to a 3-breakdown enumeration — fixed in the second pass. | Scorecard notes column; Exit Analysis table; pre-radar sector column. Corrected in `83bc382` and the follow-up commit. | Any count or ordering over a table gets a `grep -c` / sort in the report, like the totals. |
...
This file was itself fact-checked by qa-auditor on 2026-09-12 (verdict ACCURATE-w-CORRECTIONS, 15 findings folded: SPY history P5, FMC S2D history J5, P3 row count, probe archive, `c5394a8` traceability, timestamps, PARA gap, J9 count, J10 residual).
