...
- **Decisión (actualizada 2026-08-16, override del operador):** el gate falló, pero el operador decidió construir el sidecar §3.5 de todas formas como **anotación pura** (banda del modelo junto a la banda random-walk, sin cablear al Journal, sin filtrar nada). Brief completo para la siguiente sesión: `docs/timesfm-fase1-spec.md` §10 (construir `weekly.sh` + timer, correr sobre W33: 24 señales). Fases 2–3 siguen sin precondición. El runtime `/opt/timesfm` (~1.7 GB) y `scripts/tfm/` se conservan por si se quiere re-correr con otro horizonte/contexto; `rm -rf /opt/timesfm` lo elimina sin efectos.
