...
> **Estado (2026-08-16):** Fase 1 ejecutada según `docs/timesfm-fase1-spec.md` — **gate FAIL (G1)** en tres corridas (DB mixta con exclusión de costura · ventana solo-Polygon · copia de base única corregida por dividendos/splits): TimesFM está calibrado pero no supera al random walk. Re-test post-hoc contra una banda naive más fuerte (σ52): **sin mejora** en ninguna variante. **TimesFM DESCARTADO del Radar (ruling del operador, 2026-08-16)**; Fases 2–3 no proceden; el override de §3.5 quedó rescindido el mismo día. Ver §9.
...
- **Re-test contra banda σ52 (2026-08-16, misma sesión):** antes de construir el sidecar por override, un diagnóstico sobre las 2,851 filas mostró que la P50 del modelo sí se mueve (sd ≈ 3%/4 sem) pero no correlaciona con lo que pasa (corr +0.05; aciertos de dirección 52% vs 56% de "siempre arriba"), y que su **ancho** de banda sí supera al σ26 cuando se le quita la deriva *y se simetriza* (variante ancho-simétrico: ΔPB −0.0004 [−0.0007, −0.0001], solo en la población completa; quitar la deriva sola: −0.0003 [−0.0006, +0.0000], no significativo — `report-sigma26.md`) — pero una **σ de 52 semanas** hace lo mismo (σ52 vs σ26: −0.0004 [−0.0006, −0.0001]) y TimesFM vs EWMA(0.97) es empate. Re-test formal (`scripts/tfm/retest-sigma52.py --clean-from`, `results/tfm-backtest/2026-08-16-sigma52/report.md`; 2,821 filas — 30 descartadas porque su ventana σ52 alcanza el tramo no reconciliado del ticker) con σ52 como baseline: TimesFM tal cual **+0.0004 [−0.0002, +0.0009]** (todas) / **+0.0004 [−0.0004, +0.0011]** (post-2025); sin deriva +0.0001 / +0.0002; ancho simétrico −0.0000 / +0.0001 — **ninguna variante mejora**. La banda σ52 cubre 81.0% / 79.4% (calibrada) y el σ26 del spec resulta significativamente peor que σ52 (+0.0004 [+0.0001, +0.0006]): la única "ventaja" que TimesFM llegó a mostrar era σ52-vs-σ26. Chequeo de valor como anotación (resultado por quintil de σ52, solo informativo): P(r>0) 59.6%/59.4% en los dos quintiles de baja volatilidad vs 52.0–55.2% en Q3–Q5, pero Q1 − Q5 = +4.4 pp con IC 95% (clusterizado por semana) [−3.0, +11.4] pp — **no distinguible de ruido**; en cualquier caso no es filtro (lógica congelada). La banda equivalente sin modelo queda como one-shot `scripts/sigma52-band.ts --week <ISO>` (imprime markdown, solo lectura, no cableada; ejemplo W33 en `results/tfm-backtest/2026-08-16-sigma52/w33-sigma52-band.md`).
- **Decisión (final, 2026-08-16, ruling del operador):** "si no hay mejora sobre σ52, descartar TimesFM del Radar" → **descartado**. Override de §3.5 rescindido; nada se cableó nunca (sin unit, sin timer, sin Journal/Telegram, `src/` intacto, `radar.db` solo lectura). `scripts/tfm/` se conserva como harness de backtest archivado (cualquier forecaster futuro pasa por el mismo gate, con σ52 como baseline); `/opt/timesfm` (~1.7 GB) ya no lo usa el Radar — `rm -rf /opt/timesfm` lo elimina sin efectos (conservar si TimesFM se apunta a series de negocio en otro proyecto). Reabrir solo con fine-tuning sobre las semanas-señal con split temporal estricto, o con otro objetivo con estructura temporal real; re-tests zero-shot sobre precio quedan cerrados (regla de 3 strikes).
