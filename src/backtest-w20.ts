...
const ALL_SECTORS = ["XLU","XLI","XLP","XLE","XLF","XLV","XLB","XLY","XLK","XLC","XLRE","IBB","XBI"];
const sectorNames: Record<string, string> = {
  XLU:"Utilities", XLI:"Industrials", XLP:"ConsStaples", XLE:"Energy",
  XLF:"Financials", XLV:"Healthcare", XLB:"Materials", XLY:"ConsDiscret",
  XLK:"Technology", XLC:"CommServices", XLRE:"RealEstate", IBB:"Biotech", XBI:"SmallBiotech"
};
...
async function main() {
  ...
  console.log("Running W20 backtest over ALL sectors ("+ALL_SECTORS.join(", ")+")");
  ...
  for (const sector of ALL_SECTORS) {
    ...
    if(!tickers.length) { console.log("\n=== "+sector+" — no tickers in DB (run fetch first) ==="); continue; }
    console.log("\n=== "+sector+" ("+sectorNames[sector]+") — "+tickers.length+" tickers ===");
    for (const ticker of tickers) {
      const candles=loadFromDb(db, ticker);
      if(candles.length<50) { console.log("  "+ticker+": insufficient data ("+candles.length+" bars)"); skippedNoData.push(ticker); continue; }
  ...
  writeFileSync(RESULTS_DIR+"/backtest_w20_scorecard.csv", scoreCsv);
  ...
  writeFileSync(RESULTS_DIR+"/backtest_w20_outcomes.csv", outCsv);

  console.log("\n\n=== TOP 20 — W20 Universe ===");
  console.log("Rank | Ticker | Sector        | Sigs | HR8W  | Ret8W  | DD     | Score");
  console.log("-----|--------|---------------|------|-------|--------|--------|------");
  for (const r of tickerResults.slice(0,20)) {
  ...
  if(skippedNoData.length) console.log("\nSkipped (no data — run fetch): "+skippedNoData.join(", "));
  ...
  console.log("Total: "+allOutcomes.length+" outcomes | Saved: "+RESULTS_DIR+"/backtest_w20_scorecard.csv");
  db.close();
}
