...
function loadCache(ticker: string): WeeklyCandle[] | null {
  ...
  const raw = JSON.parse(readFileSync(path, "utf-8")) as { candles: WeeklyCandle[] };
  return raw.candles;
}

function computeMaxDrawdown(candles: WeeklyCandle[], fromIdx: number, weeks: number): number {
  const endIdx = Math.min(fromIdx + weeks, candles.length - 1);
  const entryClose = candles[fromIdx].close;
  let maxDD = 0;
...
function computeReturn(candles: WeeklyCandle[], fromIdx: number, weeks: number): number {
  const targetIdx = Math.min(fromIdx + weeks, candles.length - 1);
  if (targetIdx <= fromIdx) return 0;
  return (candles[targetIdx].close - candles[fromIdx].close) / candles[fromIdx].close;
}

function computeAoLag(bars: ReturnType<typeof calculateIndicators>, signalIdx: number): number {
...
async function main() {
  ...
  for (const [etf, { name, tickers }] of Object.entries(SECTOR_COMPONENTS)) {
    ...
    for (const ticker of tickers) {
      ...
      for (const sig of signals) {
        ...
        if (candleIdx + 12 >= candles.length) continue; // need 12W forward

        const ret4W = computeReturn(candles, candleIdx, 4);
        const ret8W = computeReturn(candles, candleIdx, 8);
        const ret12W = computeReturn(candles, candleIdx, 12);
        const maxDD = computeMaxDrawdown(candles, candleIdx, 8);
      ...
      const hitRate8W = outcomes.filter((o) => o.ret8W > 0).length / outcomes.length;
      const avgReturn4W = outcomes.reduce((s, o) => s + o.ret4W, 0) / outcomes.length;
      const avgReturn8W = outcomes.reduce((s, o) => s + o.ret8W, 0) / outcomes.length;
      const avgReturn12W = outcomes.reduce((s, o) => s + o.ret12W, 0) / outcomes.length;
      const avgMaxDD = outcomes.reduce((s, o) => s + o.maxDD, 0) / outcomes.length;
      const avgAoLag = outcomes.reduce((s, o) => s + o.aoLag, 0) / outcomes.length;
      const cleanOutcomes = outcomes.filter((o) => o.clean);
      const cleanHitRate = cleanOutcomes.length > 0
        ? cleanOutcomes.filter((o) => o.ret8W > 0).length / cleanOutcomes.length
        : 0;

      // Composite score (same formula as Phase 2 for comparability)
      const samplePenalty = outcomes.length < 10 ? 0.5 : 1.0;
      const score = (hitRate8W * 0.4 + cleanHitRate * 0.3 + Math.min(avgReturn8W * 5, 0.3))
  ...
  // Write outcomes CSV
  const outcomesHeader = "ticker,date,sector,ret4W,ret8W,ret12W,maxDD,aoLag,aoBottomDepth,aoRecovery,macroFilter,clean";
  const outcomesRows = allOutcomes.map((o) =>
    [
      o.ticker, o.date, o.sector,
      o.ret4W.toFixed(4), o.ret8W.toFixed(4), o.ret12W.toFixed(4),
      o.maxDD.toFixed(4), o.aoLag,
      o.aoBottomDepth, o.aoRecovery.toFixed(6),
      o.macroFilter, o.clean,
    ].join(",")
  );
  writeFileSync(`${RESULTS_DIR}/s2_outcomes.csv`, [outcomesHeader, ...outcomesRows].join("\n"));

  // Write scorecard CSV
  const scorecardHeader = "ticker,sector,sectorName,totalSignals,hitRate8W,avgReturn4W,avgReturn8W,avgReturn12W,avgMaxDD,avgAoLag,cleanSignals,cleanHitRate,score,bestReturn8W,worstReturn8W";
  const scorecardRows = tickerResults
    .sort((a, b) => b.score - a.score)
    .map((r) =>
      [
        r.ticker, r.sector, r.sectorName, r.totalSignals,
        r.hitRate8W.toFixed(4), r.avgReturn4W.toFixed(4),
        r.avgReturn8W.toFixed(4), r.avgReturn12W.toFixed(4),
        r.avgMaxDD.toFixed(4), r.avgAoLag.toFixed(1),
        r.cleanSignals, r.cleanHitRate.toFixed(4),
        r.score.toFixed(4), r.bestReturn8W.toFixed(4), r.worstReturn8W.toFixed(4),
      ].join(",")
    );
  writeFileSync(`${RESULTS_DIR}/s2_scorecard.csv`, [scorecardHeader, ...scorecardRows].join("\n"));

  console.log("\n=== S2 BACKTEST SUMMARY ===");
  console.log(`Tickers analyzed: ${totalTickers - missingCache}`);
  console.log(`Total S2 signals: ${allOutcomes.length}`);
  const overallHR = allOutcomes.filter((o) => o.ret8W > 0).length / allOutcomes.length;
  const overallDD = allOutcomes.reduce((s, o) => s + o.maxDD, 0) / allOutcomes.length;
  const overallRet = allOutcomes.reduce((s, o) => s + o.ret8W, 0) / allOutcomes.length;
  const overallLag = allOutcomes.reduce((s, o) => s + o.aoLag, 0) / allOutcomes.length;
  ...
  console.log(`  ${RESULTS_DIR}/s2_scorecard.csv (${tickerResults.length} tickers)`);
}
