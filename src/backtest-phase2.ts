...
async function main() {
  ...
  for (const [etf, { name, tickers }] of Object.entries(SECTOR_COMPONENTS)) {
    ...
    for (const ticker of tickers) {
      ...
      for (const sig of signals) {
        ...
        // Need at least 12 weeks forward
        if (candleIdx + 12 >= candles.length) continue;
        ...
        const maxDD = computeMaxDrawdown(candles, candleIdx, 12);
      ...
      const cleanHitRate = cleanSigs.length > 0 ? cleanHits / cleanSigs.length : 0;
  ...
  console.log("Rank | Ticker | Sector      | Signals | HR8W  | Ret8W  | MaxDD   | AoLag | Score");
  console.log("-----|--------|-------------|---------|-------|--------|---------|-------|------");
  ...
  // Sector summary
  console.log("\n\n=== SECTOR SUMMARY ===");
  for (const etf of Object.keys(SECTOR_COMPONENTS)) {
  ...
  const bullHR = bullSignals.filter(o => o.ret8W > 0).length / (bullSignals.length || 1);
  const bearHR = bearSignals.filter(o => o.ret8W > 0).length / (bearSignals.length || 1);

  console.log("\n=== MACRO FILTER ANALYSIS ===");
  console.log(`Bull market signals (SPY > SMA40): ${bullSignals.length} → Hit Rate ${(bullHR * 100).toFixed(1)}%`);
  console.log(`Bear market signals (SPY < SMA40): ${bearSignals.length} → Hit Rate ${(bearHR * 100).toFixed(1)}%`);
